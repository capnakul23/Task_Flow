...
export default function KanbanColumn({ status, label, issues = [], canCreate = false, onCreateIssue, onIssueClick }) {
  return (
    <section className="flex min-w-[280px] max-w-[340px] flex-1 flex-col">
      ...
      <div className="flex flex-col gap-2">
        ...
        ) : issues.map((issue) => <IssueCard key={issue.id} issue={issue} onClick={() => onIssueClick?.(issue)} />)}
      </div>
    </section>
  )
}
